...
renders nucleus centroids coloured by cell type on top of the tissue
thumbnail:
...
`mean_intensity` and `nearest_neighbor_distance`. Spatial coordinates
are available via
[`spatialCoords()`](https://rdrr.io/pkg/SpatialExperiment/man/SpatialExperiment-methods.html)
as `x_centroid` and `y_centroid`.
