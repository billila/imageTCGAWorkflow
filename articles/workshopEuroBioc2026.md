...
1.  Use
    ...
    to browse the image feature catalogue and filter by cancer type,
    pipeline, and file format
2.  Import HoVerNet nuclei data into a `SpatialExperiment` using
    `HoverNet() |> import()` and explore
...
3.  Visualize nuclei segmentation on a tissue thumbnail using
    [`plotHoverNetH5ADOverlay()`](https://rdrr.io/pkg/HistoImagePlot/man/plotHoverNetH5ADOverlay.html)
...
## Part 2 — Ecosystem Overview (15–30 min)
...
## Part 3 - Raw Image Download (30–40 min)
...
## Part 4 — HoVerNet Nuclei Features (40–55 min)
...
## Part 5 — Prov-GigaPath Embeddings (55–70 min)
...
ProvGigaPath is a foundation model trained on large-scale pathology
image tiles that produces high-dimensional embeddings summarizing visual
...
Tile-level embeddings provide localized representations of tissue
regions.
...
Point Pattern Analysis (PPA) provides a framework to investigate the
spatial distribution of cells within histopathology images. When
HoVerNet output is imported through `ImageFeatureTCGA`, each cell is
represented with spatial coordinates and associated morphological or
intensity features. These data allow the computation of global and local
spatial autocorrelation metrics, such as Moran’s I, Local Moran’s I
(LISA), Geary’s C, and Local Spatial Heteroscedasticity (LOSH).

Here, we illustrate how to perform spatial autocorrelation analysis
starting from a `SpatialFeatureExperiment` object created from HoVerNet
...
Moran’s I quantifies the degree to which similar values cluster
spatially. A positive Moran’s I indicates spatial clustering, while
...
In this section we want show you how to integrate HoVerNet
...
Visualizing Hovernet nuclei vs tile coordinates to see that they do not
match perfectly.
...
## Part 6 — Multi-Modal Integration (70–80 min)
...
embed_mat <- readRDS("~/imageTCGAWorkflow/inst/extdata/ov_embeddings.rds")
rna_mat   <- readRDS("~/imageTCGAWorkflow/inst/extdata/ov_rnaseq.rds")
meth_mat  <- readRDS("~/imageTCGAWorkflow/inst/extdata/ov_methylation.rds")
