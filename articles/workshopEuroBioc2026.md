...
| 20–30 min | Data discovery with `imageTCGA` (Shiny app) | Live demo + Hand-on |
| 30–50 min | HoVerNet nuclei features: import, explore, visualize | Hands-on |
| 50–70 min | Prov-GigaPath embeddings: tile & slide level, PCA, spatial stats | Hands-on |
...
ov_slide_files <- getCatalog("provgigapath") |> 
    dplyr::filter(Project.ID == "TCGA-OV")

ov_slide_files <- ov_slide_files[1:5, ]


structure_last <- ov_slide_files |>
    getFileURLs() |>
    ProvGigaList()

rownames(structure_last) <- ov_slide_files[["tcga_barcode"]]
```
