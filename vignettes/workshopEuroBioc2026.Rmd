...
We integrate image-derived features (Prov-GigaPath slide-level embeddings)
with TCGA molecular data using MOFA+ (`MOFA2`). This identifies joint factors
of variation across modalities.

The pre-computed data used here (10 TCGA-OV samples with matched embeddings,
RNA-seq, and methylation) is shipped with the package. The code to regenerate
these files from scratch is provided below in comments.
...
<!--
## Code to regenerate RDS files (not run)
To reproduce the pre-computed files, run `data-raw/prepare_extdata.R`:

```{r mofa-prepare-data, eval=FALSE}
# Requires internet connection to download TCGA data
library(curatedTCGAData)
library(MultiAssayExperiment)

mae <- curatedTCGAData(
    diseaseCode = "OV",
    assays      = c("RNASeq2GeneNorm", "Methylation_methyl27"),
    version     = "2.0.1",
    dry.run     = FALSE
)
...
tcga_patient_id <- function(x) {
    x <- sub("\\..*$", "", x)
    sapply(strsplit(x, "-"), function(s) paste(s[1:3], collapse = "-"))
}

ov_embeddings <- read.csv(system.file("extdata",
    "emb_gigapth_OV_last_embed.csv", package = "imageTCGAWorkflow"))
ov_embeddings$patient_id <- tcga_patient_id(ov_embeddings$slide_name)

rna_mat_full  <- as.matrix(assay(mae[["OV_RNASeq2GeneNorm-20160128"]]))
meth_mat_full <- as.matrix(assay(mae[["OV_Methylation_methyl27-20160128"]]))

common <- Reduce(intersect, list(
    ov_embeddings$patient_id,
    tcga_patient_id(colnames(rna_mat_full)),
    tcga_patient_id(colnames(meth_mat_full))
))
# common <- common[1:min(10, length(common))]

# Embeddings
embed_sub  <- ov_embeddings[ov_embeddings$patient_id %in% common, ]
embed_cols <- grep("^V[0-9]+$", names(embed_sub), value = TRUE)
embed_mat  <- t(as.matrix(embed_sub[, embed_cols]))
colnames(embed_mat) <- embed_sub$patient_id
embed_mat  <- embed_mat[, common]

# RNA-seq: top 1000 most variable genes
idx_rna <- match(common, tcga_patient_id(colnames(rna_mat_full)))
rna_sub <- rna_mat_full[, idx_rna]
colnames(rna_sub) <- common
rna_sub <- rna_sub[order(apply(rna_sub, 1, var),
                         decreasing = TRUE)[1:1000], ]

# Methylation: remove probes with NA
idx_meth <- match(common, tcga_patient_id(colnames(meth_mat_full)))
meth_sub <- meth_mat_full[, idx_meth]
colnames(meth_sub) <- common
meth_sub <- meth_sub[complete.cases(meth_sub), ]

saveRDS(embed_mat, "inst/extdata/ov_embeddings.rds")
saveRDS(rna_sub,   "inst/extdata/ov_rnaseq.rds")
saveRDS(meth_sub,  "inst/extdata/ov_methylation.rds")

# Tip: replace diseaseCode = "OV" with any TCGA code (e.g. "BRCA", "LUAD")
# and swap the embeddings CSV to extend the analysis to other cancer types.
```
-->
